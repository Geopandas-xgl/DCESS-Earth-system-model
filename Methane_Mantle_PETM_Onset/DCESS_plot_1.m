...
for i = fliplr(1:num_idx)
    ...
    mix_total_serial(i,:) = cumtrapz(run(i).state.st,flux_mix(num_idx,:));
end
...
fill([t_kyr(idx.meth(1):idx.mett(2)),fliplr(t_kyr(idx.meth(1):idx.mett(2)))],[flux_mix(1,idx.meth(1):idx.mett(2)),fliplr(flux_mix(num_idx,idx.meth(1):idx.mett(2)))],FaceColor(4,:),"LineWidth",1,"EdgeColor","none","FaceAlpha",1); 
line(t_kyr(idx.meth(1):idx.mett(2)),flux_mix(mid_idx,idx.meth(1):idx.mett(2)),"Color",EdgeColor(4), "LineWidth",1.4);
...
text_norm(0.6+0.1,0.7+0.03,"CH4\_hyrat (-60 ‰)",9);
...
line_norm([0.6,0.6+0.08],[0.4+0.08/2,0.4+0.08/2],EdgeColor(4),1.2);
text_norm(0.6+0.1,0.4+0.03,"CO2\_mix (-11 ‰)",9);

ext = rectangle("Position",[ext_range(1),yrange(1),diff(ext_range),diff(yrange)],'FaceColor', EdgeColor(2), 'EdgeColor', 'none');
uistack(ext,"bottom");
...
text(7,120,"Benthic extinction","Color",EdgeColor(2),"FontName","Times","FontSize",10);
BorderLine1(gca,"right");
set(gca,"YAxisLocation","left","Xcolor","none","Xtick",[],"linewidth", 1,"FontSize",10,"FontName", "Times","TickLength",[0.015,0.03],"Layer","top");
...
POE = rectangle("Position",[POE_range(1),yrange(1),diff(POE_range),diff(yrange)],'FaceColor', "#E4DB84", 'EdgeColor', 'none');
uistack(POE, 'bottom');
ylabel({"Anoxic fraction", "(f_{anoxic}) (%)"});
...
%% 
print(gcf,"Figure\DCESS_PETM_1","-dpng","-r600");
%% 
...
%% 
figure("Position",[0,0,840,260])
% [ha,~] = tight_subplot(1,2,[0 0.17],[0.03 0.08],[0.05 0.11]);
[ha,~] = tight_subplot(1,2,[0 0.17],[0.1 0.08],[0.05 0.11]);
axes(ha(1))
...
ticklabels("y",yticks,2,"F");
ylabel('Depth (km)')
title('Low-middle latitude; λ = 60%')
box off
...
cb = colorbar('Position',[0.41 0.03 0.02 0.88]);  % Adjust as needed
...
%--- Colorbar outside right of high-latitude subplot ---
cb = colorbar('Position',[0.905 0.03 0.02 0.88]);  % Adjust as needed
cb.Label.String = 'O_2 (mmol/m^3)';
