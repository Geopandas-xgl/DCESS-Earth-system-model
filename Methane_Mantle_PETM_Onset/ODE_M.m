...
global minO minN k_Poxidw RorgLL_input mgt mixc13 lipc13
...
% org_rate = interp1(input.org.time,input.org.rate,t/sy/1000,"linear")*mgt/sy;



if ((t/sy >= input.mix.MHM(1) && t/sy <= input.mix.MHM(2) ))
     mix_rate = ((input.mix.MHM (3)*(t/sy-input.mix.MHM (1))^4*exp(-input.mix.MHM (4)*(t/sy-input.mix.MHM (1)))) + input.mix.rate_base)*mgt/sy; 
else
     mix_rate = input.mix.rate_base*mgt/sy;  
end


mix_vel(1) = mix_rate;  %
mix_vel(2) = mix_rate*(mixc13*1e-3+1)*R13pdb;
...
% lip_rate = interp1(input.lip.time,input.lip.rate,t/sy/1000,"linear")*mgt/sy;

if ((t/sy >= input.lip.MHM(1) && t/sy <= input.lip.MHM(2) ))
     lip_rate = ((input.lip.MHM (3)*(t/sy-input.lip.MHM (1))^4*exp(-input.lip.MHM (4)*(t/sy-input.lip.MHM (1)))) + input.lip.rate_base)*mgt/sy; 
else
     lip_rate = input.lip.rate_base*mgt/sy;  
end
...
%***********************************************************************%
for m = 9:20     % hydrate methen release depth ranging from 900 to 2000 m  
%***********************************************************************%    
...
% Atmospheric temperature, gasses and tracers
  ...
  elseif i==4
      kAT(i,:)  =-rVa*(asLL(4) + asHL(4) - al(9) - mdr(1)+...%-(asLL(2) + asHL(2)
           Wcarb + 2*Wsil -Worg - Volin- mix_vel(1)-lip_vel(1)); 
      %last term is volcanic input     
  elseif i==5 
      kAT(i,:)  =-rVa*( + asLL(5) + asHL(5) - al(10) - mdr(2) +...%
         (Wcarb+2*Wsil)*AT(5,1)/AT(4,1) - ...
          Worg*(R13pdb*(RCorgC13+1))-Volin*(R13pdb*(RVolC13+1))-mix_vel(2) -lip_vel(2));   
  elseif i==6 
...
car(3) = mix_vel(1);                               % C mol/s associated with mixcture in mantle and organic matter oxidation (wildfire and thawing permofrost)
